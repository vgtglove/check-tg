    ...
    def updateState(self, status):
        ...
        if status == "正在运行":
            if not self.blink_timer.isActive():
                self.blink_timer.start(300)  # 300毫秒一次闪烁
        else:
...
class ActivityResultDialog(QDialog):
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            if file_name:
                # 创建DataFrame
                data = []
                for user in self.activity_results:
                    last_seen_str = ""
                    if user.last_seen:
    ...
    def export_all_active(self):
        """导出所有活跃用户信息"""
        try:
            active_users = [a for a in self.activity_results if a.is_active]
            if not active_users:
